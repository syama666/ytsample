...
var (
	clientSecretsFile = flag.String("secrets", "conf/client_secrets.json", "Client Secrets configuration")
	cachefile         = flag.String("cache", "request.token", "Token cache file")
	configfile        = flag.String("c", "conf/local.json", "Config file by JSON")
)
...
func Execute() {
	...
	youtube, err := yt.New(httpClient)
	if err != nil {
		log.Fatalf("Error creating YouTube client: %v", err)
	}

	apiCall := youtube.PlaylistItems.List("id")
	log.Println("=========")
	log.Println(apiCall)
	log.Println("=========")
	response, err := apiCall.Do()
	if err != nil {
		log.Fatalf("Error making API call to list playlistitems: %v \n", err)
		log.Fatalf("MOGEEEEEError making API call to list playlistitems: %v \n", err.Error())
	}

	for _, item := range response.Items {
		log.Printf("hogehoge :: %v\n", item)
	}
